...
function fxPickList($compile, $templateCache,$filter){
		...
		controller: function($scope, $element, $attrs, $parse){
			...
			$scope.leftShift=function(){
				 var models=modelFn($scope);
				 	...
				 	//console.log(models);//right total values 
				 //	console.log($scope.picklist_dest); // right side select value 
				for (var i=0; i<$scope.picklist_dest.length; i++){
					$scope.srcoptions.push($scope.picklist_dest[i]);
					$scope.toptions.push($scope.picklist_dest[i]);
				}
				//$scope.toptions = [];
				//var toption = $filter('orderObjectBy')($scope.srcoptions,'PROCEDURE_GROUP');
				...
				$scope.removeLeftFromRight($scope.picklist_dest, $scope.tselected);
				...
				//});
				// $scope.destoptions left side values 

				angular.forEach($scope.destoptions, function(item){
					models.push($scope.itemToValue(item));
				});
				//$scope.toptions = $scope.srcoptions;
				/*$scope.$apply(function () {
					$scope.toptions = $scope.srcoptions;
				});*/
				//$scope.toptions = $scope.srcoptions;
				//console.log($scope.toptions);
				//console.log($scope.toptions);

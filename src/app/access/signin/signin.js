...
.controller('signinCtrl', ['$rootScope','$scope','$http','$state','$timeout','$cookieStore','authService','$localStorage','$location',  function($rootScope,$scope,$http,$state,$timeout,$cookieStore,authService,$localStorage,$location) {
  ...
  $scope.user = {};
    ...
    $scope.login = function()
    {
      ...
      $http({
                url: apiUrl+'signin?tableName=users&fields=*', //Your url will be like this.
                method: 'post',
                data: $scope.user,
                headers: {'Content-Type': 'application/json'}
                })
                  ...
                  if(data.length>0) 
                  {
                    ...
                    // $cookieStore.put('userName',data[0]);
                     ...
                     mixpanel.people.set({
                       ...
                       $http({
                url:apiUrl+"getCptTable?tableName=idndetails&filter= where IDN3 = '"+ data[0].hospital +"' limit 1&fields=IDN3,IDNNAME,file_format", 
                method: 'get',
                data:'' ,
                headers: {'Content-Type': 'application/json'}
                })
              .success(function(idn_data)
                {
                 $localStorage.file_format = (idn_data.length>0 && angular.isDefined(idn_data[0].file_format) && idn_data[0].file_format!=null)?idn_data[0].file_format:0;

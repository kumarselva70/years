...
.controller('Listpatients', ['$scope', '$http', '$state', '$stateParams', '$modal', '$cookieStore', '$localStorage', '$timeout', 'DTOptionsBuilder', '$rootScope', function($scope, $http, $state, $stateParams, $modal, $cookieStore, $localStorage, $timeout, DTOptionsBuilder, $rootScope) {
        ...
        $scope.selcstatus = ['All Records', 'Completed', 'Draft','Uploaded'];
        $scope.selectProc = [{name:'All Procedures',value:"!=''"},{name:'Benchmarked Only',value:"!='Non Benchmarked'"},{name:'Non Benchmarked',value:"='Non Benchmarked'"}];
        $scope.patientFilters.procstatus = $scope.selcstatus[0];
        $scope.patientFilters.selectProcedure = $scope.selectProc[0];
  ...
  $scope.$watch('currentPage + numPerPage', function() {
    if(angular.isDefined($scope.rowLength))
    {
        ...
        $scope.updateTableData = function(searVal) {
            ...
            $scope.offset = 0;
            whereCondition = "";
            //console.log($scope.patientFilters.selectProcedure);

if($scope.patientFilters.procstatus=='Uploaded'){$scope.stsus = 'upload_status'; } else{ $scope.stsus = 'status'; }
            if (searVal === 0) {
                $scope.created_onFilter = '';
            }
            ...
            whereCondition = $scope.stsus+ " " + $arr[$scope.patientFilters.procstatus] + " " + IDNVALUE + " " + $scope.created_onFilter;
            ...
            if (!angular.isUndefined($scope.patientFilters.selectProcedure) && $scope.patientFilters.selectProcedure.value!=="!=''") {
                whereCondition += " and PROC " + $scope.patientFilters.selectProcedure.value + " ";
            }

...
.controller('AddCenterscntrl', ['$scope','$http','$state','$modal','DTOptionsBuilder','$timeout','$stateParams','$localStorage', '$rootScope','$filter', function($scope,$http,$state,$modal,DTOptionsBuilder,$timeout,$stateParams,$localStorage,$rootScope,$filter) {
 ...
 $scope.toptions=[];
  ...
  $scope.tselected=[];
  $scope.filesformat = csv_text;//[{name:"None",value:1},{name:"Advantx",value:2}];
  ...
  $scope.previous = function() {
              ...
              $scope.centerSteps.cpt = false;
              $scope.centerSteps.Center = true;
            };
     ...
     if ($scope.addcenter.$valid)
        {
          ...
          if(angular.isUndefined($scope.addCenter.file_format) || angular.isUndefined($scope.addCenter.file_format['value']))
          {
            $scope.errortype = 1;
                    $scope.alrttype = "danger";
                    $scope.disableButton = 0;
                    $scope.msg = "Please Check File format!!";
                     return false;
          }
          ...
          $scope.addcenterDBfields.file_format= (angular.isDefined($scope.addCenter.file_format))?$scope.addCenter.file_format['value']:"";
        ...
        $scope.cptfavDBfields={};
        $scope.cptfavDBfields.idn3 =$scope.addCenter.IDN3;
        //$scope.cptfavDBfields.cpt_codes = ($scope.tselected.length>0)?JSON.stringify($scope.tselected):[];
        $scope.cpt_codes_fave = [];
        if($scope.tselected.length>0)
        {

          angular.forEach($scope.tselected,function(values)
          {
            $scope.cpt_codes_fave.push(values.value);
          });
        }
        
        $scope.cptfavDBfields.cpt_codes = ($scope.cpt_codes_fave.length>0)?JSON.stringify($scope.cpt_codes_fave):'[]';

//console.log($scope.cptfavDBfields.cpt_codes);
        mixpanel.track("Add Center", {
          ...
          if($scope.csptfaveAlready>0)
          {
            cptfavurl = apiUrl+"update?tablename=cpt_code_fav&filter=where idn3 ='"+$scope.addCenter.IDN3+"'";  
            $scope.cptfavDBfields.updated_on= moment().format("YYYY-MM-DD HH:mm");
          }
          else
          {
            cptfavurl = apiUrl+'insert?tablename=cpt_code_fav';
            $scope.cptfavDBfields.created_on= moment().format("YYYY-MM-DD HH:mm");
          }
                                ...
                                if(data>0)
                                {
                                        $http({
                                              ...
                                              .success(function(data)
                                               ...
                                               url: cptfavurl,
                                              method: 'post',
                                              data:$scope.cptfavDBfields,
                                              headers: {'Content-Type': 'application/json'}
                                              })
                                              .success(function(datacpt)
                                               {
                                                //console.log(datacpt);
                                                });
  ...
  };//save center function 

$scope.picklilData = function(ind,code)
{
$scope.toptions = [];
$scope.tselected = [];
var ascf = (code)?code:'PROCEDURE_GROUP';
  $http({
                        url: apiUrl + "getCptTable?tableName=cpt_code_fav&filter=where idn3 ='"+ind+"'  &fields=cpt_codes,idn3", //Your url will be like this.
                        method: 'get',
                        data: '',
                        headers: {
                    ...
                    .success(function(datafav) {
                     ...
                     $http({
                        url: apiUrl + "getCptTable?tableName=cpt_codes&filter=where PROCEDURE_GROUP !='Non Benchmarked' order by " + ascf+ " asc &fields=DESCRIPTION,CODE,PROCEDURE_GROUP", //Your url will be like this.
                        method: 'get',
                        data: '',
                        headers: {
                            'Content-Type': 'application/json'
                    ...
                    .success(function(data) {
                        ...
                        var cpt_fav_codes = [];
                        if (datafav.length > 0) {
                          $scope.csptfaveAlready = 1;
                        cpt_fav_codes = JSON.parse(datafav[0].cpt_codes,true);
                      }
                      //console.log(cpt_fav_codes);
                        if (data.length > 0) {
                            $scope.cptCodes = [];
                            var i = 0;
                            angular.forEach(data, function(value, key) {
                                   
                                    // angular.forEach(cpt_fav_codes,function(cpcode_fav)
                                   // {
                                      if(cpt_fav_codes.includes(value.CODE) === true)
                                    {
                                      $scope.tselected.push({
                                        name:  value.CODE +" - " + value.PROCEDURE_GROUP + " ",
                                    PROCEDURE_GROUP: value.PROCEDURE_GROUP,
                                    BENCHMARK: value.BENCHMARK,
                                    value: value.CODE});
                                    }
                                    //});

                                    if(cpt_fav_codes.includes(value.CODE) === false)
                                    {
                                       $scope.toptions.push({
                                      name:  value.CODE +" - " + value.PROCEDURE_GROUP + " ",
                                      PROCEDURE_GROUP: value.PROCEDURE_GROUP,
    ...
    $scope.sortByLefttSideProc = function(key)
    {
       
       var toption = $filter('orderObjectBy')($scope.toptions,key);
       
      $scope.toptions  = [];
      $scope.toptions = toption;

    };

    
    $scope.sortByRightSideProc = function(key)
    {
       var tselect = $filter('orderObjectBy')($scope.tselected,key);
       
       $scope.tselected  = [];

      $scope.tselected = tselect;
    };
   ...
   if($stateParams.editId!=='' && $stateParams.editId.length>0)

          {
            ...
            $http({
                url: apiUrl+'getCptTable?tableName=idndetails&filter=where id= '+$stateParams.editId+' order by IDNNAME asc&fields=status,IDN3,IDNNAME,id,PATINT,file_format&limit=1', 
                method: 'get',
                data:'' ,
                headers: {'Content-Type': 'application/json'}
                })
              .success(function(data)
                {
                  ...
                  if(data.length>0) 
                  {
                    $scope.filedaname = [{"1":"None","2":"Advantx"}];
                    $scope.addCenter.IDNNAME = data[0].IDNNAME;
                    $scope.addCenter.IDN3 = data[0].IDN3;
                    $localStorage.IDN = data[0].IDN3;
                    $scope.copyIDN3 = angular.lowercase(data[0].IDN3);
                    $scope.addCenter.status = data[0].status;
                    
                    $scope.addCenter.file_format = angular.isDefined($scope.filedaname[0][data[0].file_format])?{name:$scope.filedaname[0][data[0].file_format],value:data[0].file_format}:"";

                     $scope.picklilData(data[0].IDN3);
          ...
          {
            $scope.picklilData($localStorage.IDN);
          }

...
.controller('elementsCtrl', ['$stateParams', '$scope', '$timeout', '$http', '$state', '$cookieStore', '$localStorage', '$filter', 'popupservice', '$modal','$rootScope', '$sce','$q', function($stateParams, $scope, $timeout, $http, $state, $cookieStore, $localStorage, $filter, popupservice, $modal,$rootScope, $sce,$q) {
...
//<label type="button" class="col-sm-4" data-dismiss="modal" ></label><label type="button" class="alert alert-danger col-sm-4" data-dismiss="modal" ng-show="errorcls">Please Enter Delete Text</label>
        ...
        popupservice.medicalrecords.PROC = (angular.isDefined($scope.medicalRecord.PROC))?$scope.medicalRecord.PROC['DESCRIPTION']:"";
         ...
         $scope.popupFunction = function(id)
         {
             ...
             $scope.modalInstance = $modal.open({
                                    template: '<div class="modal-content"><div class="modal-header"> Post-discharge Telephone Interview</div><div class="modal-body">Continue to the Patient Interview?</div><div class="modal-footer"><button type="button" class="btn btn-primary col-sm-5" data-dismiss="modal" ng-click="yes(1)"> Yes, Continue to Patient Interview</button><button type="button" class="btn btn-warning col-sm-4"  data-dismiss="modal" ng-click="close(0)">No Patient Interview needed</button><button type="button" class="btn btn-info laderPading" data-dismiss="modal" ng-click="close(0)"> No, I will do it later</button></div>',
           ...
           $scope.ErrorFileds = [];
            ...
            if (stat === 0) {
                ...
                if (angular.isUndefined($scope.medicalRecord.ANESTHESIA)) {
                   // $scope.submiterrorSubmit = 1;
                    $scope.ErrorFileds.push('ANESTHESIA');
                   // return false;
                }

                if (angular.isUndefined($scope.medicalRecord.PROC) || ($scope.medicalRecord.PROC['CODE'] === '' || $scope.medicalRecord.PROC==='') ) {
                    //$scope.submiterrorSubmit = 1;
                    //$scope.PrimaryProcedure = 1;

                    $scope.ErrorFileds.push('Primary Procedure');
                    //return false;
                ...
                if(angular.isDefined($scope.medicalRecord.ORIN) && angular.isDefined($scope.medicalRecord.OROUT))
                {
                    ...
                    {
                        //$scope.submiterrorSubmit = 1;
                        $scope.ErrorFileds.push('please Check Orout Time');
                        //return false;
                    }
                }
                ...
                if ((angular.lowercase($scope.medicalRecord.AntibioNam.value) !== 'none') && ($scope.medicalRecord.AntibioTim == '00:00' || $scope.medicalRecord.AntibioTim === '') && (angular.lowercase($scope.medicalRecord.AntibioNam) !== 'none' && $scope.medicalRecord.AntibioTim === '')) {
                    ...
                    {
                        $scope.ErrorFileds.push('Time of Inital dose');
                    //return false;    
                    }
                ...
                if ($scope.medicalRecord.PV1 == 'Y') {
                    if (angular.isUndefined($scope.medicalRecord.PR1) || $scope.medicalRecord.PR1 === '') {
                        $scope.PR1true = 1;
                        //$scope.submiterrorSubmit = 1;
                        $scope.ErrorFileds.push('Pain relieved');
                        return false;
                    }
                }
            ...
            if ((angular.isUndefined($scope.medicalRecord.PROC)|| $scope.medicalRecord.PROC==='') && stat === 1) {
            ...
            if ($scope.personal.$valid || stat === 1) {
                ...
                $scope.medrecidn3DBFields.CPT = (angular.isDefined($scope.medicalRecord.PROC)) ? $scope.medicalRecord.PROC['CPTCODE'] : "";
                ...
                var AntibioTim = (angular.isDefined($scope.medicalRecord.AntibioTim) && $scope.medicalRecord.AntibioTim!=='') ? $scope.medicalRecord.AntibioTim.replace(':', '') : "0000";
                ...
                $scope.medrecidn3DBFields.DISPOSITIO = (angular.isDefined($scope.medicalRecord.DISPOSITIO) && $scope.medicalRecord.DISPOSITIO!=='null') ? $scope.medicalRecord.DISPOSITIO : "";
               ...
               // $scope.medrecidn3DBFields.patient_int = ($scope.medicalRecord.patient_int !== '-1') ? $scope.medicalRecord.patient_int : "0";
                ...
                if ($stateParams.editId !== '') {
                    ...
                    $scope.medrecidn3DBFields.edited_by = created_id;
                    ...
                    $scope.medrecidn3DBFields.created_on = moment().format("YYYY-MM-DD HH:mm");
                     ...
                     $scope.medrecidn3DBFields.created_by = created_id;
                }
                    ...
                    .success(function(data) {
                        ...
                        if (data.insertId > 0) {
                           ...
                           /* }
                            ...
                            $scope.msg = 'Medical record cannot be saved due to duplicate PIN with the same date of service.';
    ...
    //console.log(data[0]);
                        ...
                        $scope.AntibioTimINT = (data[0].AntibioTim!=null)?data[0].AntibioTim:'';
                        $scope.medicalRecord.DOP = (data[0].DOP!=null)?data[0].DOP.substr(0, 10):'';//(angular.isDefined(data[0])) ? moment(data[0].DOP).format('MM-DD-YYYY') : "";
                        ...
                        $scope.MedDefDisable = data[0].status;
                        popupservice.medicalrecords.PROC = (angular.isDefined(data[0].PROC))?data[0].PROC:"";

                        var ag = "0"+data[0].AGE;
                        $scope.medicalRecord.AGE = (data[0].AGE>9)?parseInt(data[0].AGE,10):(ag!="0null")?ag:'';

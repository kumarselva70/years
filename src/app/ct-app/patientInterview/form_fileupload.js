...
.controller('fileuploadCtrl', ['$scope', '$state', '$stateParams', '$http', '$timeout', 'popupservice', '$localStorage', function($scope, $state, $stateParams, $http, $timeout, popupservice, $localStorage) {
        ...
        $http({
            ...
            .success(function(data) {
                ...
                if (data.length > 0) {
                    ...
                    if (data[0].PHOME == 1 || data[0].PHOME == "Y") {
                ...
                $http({
                    ...
                    .success(function(data) {
                        if (data.length > 0) {
                            ...
                            $scope.patn2StatusDis = data[0].patient_int;
                            ...
                            //console.log($scope.patientinterview.status);
                        } // medrecidn3  if loop
    ...
    $scope.saveInterview = function(stat) {
        ...
        if (stat === 0) {
            ...
            if (($scope.instructionss.length <= 0 && $scope.value.length <= 0)) {
        ...
        if ($scope.interview.$valid || stat === 1) {
            ...
            //$scope.patint2.PHOME = (angular.isDefined($scope.patientinterview.PHOME)) ? $scope.patientinterview.PHOME : "";
             $scope.patint2.PHOME = (($scope.patientinterview.PHOME=='1')) ? "Y" : "N";
            ...
            $scope.patint2.RELIEFNUM = (angular.isDefined($scope.patientinterview.RELIEFNUM) && $scope.patientinterview.RELIEFNUM!==0) ? $scope.patientinterview.RELIEFNUM : "";
            var RELIEFNUM = [{1:'Completely Relieved',2:'Completely Relieved',3:'Completely Relieved',4:'Completely Relieved',5:'Greatly Relieved ',6:'Greatly Relieved',7:'Somewhat Relieved',8:'Somewhat Relieved',9:'Not Relieved',10:'Not Relieved',"":"Clear",clear:''}];
            $scope.patint2.RELIEF  = (angular.isDefined(RELIEFNUM[0][$scope.patint2.RELIEFNUM]))?RELIEFNUM[0][$scope.patint2.RELIEFNUM]:"";
            ...
            if ($scope.patient_id > 0) {
                ...
                $scope.patint2.edited_by = created_id;
                url = 'update?tablename=patint2&filter=where id = ' + $scope.patient_id;
            } else {
                $scope.patint2.created_on = moment().format("YYYY-MM-DD HH:mm");
                $scope.patint2.created_by = created_id;
    ...
    $scope.checkBox = function(choice,choice1,choice2) {
        ...
        {
			$scope.value.push(choice1);
        }
        else if(choice2 === true)
        {
        $scope.value.push(choice2);
        }
    ...
    $scope.instructions = function(choice1,choice2) {
        $scope.instructionss = $scope.instructionss || [];

        if (choice1 === true) {

            $scope.instructionss.push(choice1);

        }
        else if(choice2 === true)
        {
			$scope.instructionss.push(choice2);
        }
